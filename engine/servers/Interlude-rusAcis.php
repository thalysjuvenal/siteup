...
$config["QUERY_LOGIN_1"] = "SELECT a.login, a.password, a.access_level, CASE WHEN (SELECT CONCAT(email, ';', accessLevel, ';', vip_end, ';', status) FROM icp_accounts WHERE login = a.login) IS NULL THEN '' ELSE (SELECT CONCAT(email, ';', accessLevel, ';', vip_end, ';', status) FROM icp_accounts WHERE login = a.login) END AS icp_table FROM accounts AS a WHERE a.login = ?";
$config["QUERY_LOGIN_2"] = "SELECT * FROM accounts WHERE login = ?";
...
$config["QUERY_SELECT_ITEM_STATS_AUGMENTATIONS"] = "SELECT * FROM augmentations WHERE item_oid = ?";
$config["QUERY_UPDATE_ITEM_STATS_AUGMENTATIONS"] = "UPDATE augmentations SET item_oid = ? WHERE item_oid = ?";
...
$config["QUERY_RANKING_GRANDBOSSES_2"] = "SELECT s.respawn_time FROM spawn_data AS s, icp_npc_interlude AS i WHERE REPLACE(LOWER(i.name), ' ', '_') = s.name AND i.id = ?";
...
$config["QUERY_RANKING_RAIDBOSSES_2"] = "SELECT s.respawn_time FROM spawn_data AS s, icp_npc_interlude AS i WHERE REPLACE(LOWER(i.name), ' ', '_') = s.name AND i.id = ?";
$config["QUERY_RANKING_RAIDPOINTS"] = "SELECT c.*, (SELECT SUM(points) FROM character_raid_points WHERE char_id = c.obj_Id) AS raid_points, IF((SELECT clan_name FROM clan_data WHERE clan_id = c.clanid) IS NULL, 'n/a', (SELECT clan_name FROM clan_data WHERE clan_id = c.clanid)) AS clan FROM characters AS c WHERE c.accesslevel = '0' ORDER BY raid_points DESC, char_name ASC LIMIT {MAX_LIMIT}";
...
$config["QUERY_GET_CHARACTER_QUESTS"] = "SELECT c.*, (SELECT char_name FROM characters WHERE obj_Id = c.charId) AS char_name FROM character_quests AS c WHERE c.charId = ? AND (SELECT account_name FROM characters WHERE obj_Id = c.charId) = ? AND value in (SELECT MAX(value) FROM character_quests WHERE charId = c.charId GROUP by name)";
